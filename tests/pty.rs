#[cfg(unix)]
#[test]
fn terminal_binary_accepts_a_prompt_and_restores_the_screen() {
    ...
    writer.write_all(b"phase zero prompt\r\x04").unwrap();
    writer.flush().unwrap();
    ...
    assert!(output.contains("phase zero prompt"), "{output:?}");
    assert!(
